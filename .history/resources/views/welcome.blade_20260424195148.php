...
@section('content')
    ...
    <div class="modal-filter" id="modalGoal">
    <div class="filter-card">
        <h3 style="text-align: center; margin-bottom: 25px; font-weight: 900; color: white;">TARGET BARU</h3>
        <form action="{{ route('goals.store') }}" method="POST">
            @csrf
            <div style="margin-bottom: 15px;">
                <label style="font-size: 10px; color: #555; font-weight: 800;">NAMA TARGET</label>
                <input type="text" name="name" placeholder="Contoh: Beli Sepatu" required style="width: 100%; background: #1A1A1A; border: 1px solid #333; color: white; padding: 15px; border-radius: 15px; margin-top: 5px;">
            </div>
            <div style="margin-bottom: 15px;">
                <label style="font-size: 10px; color: #555; font-weight: 800;">NOMINAL TARGET (RP)</label>
                <input type="number" name="target_amount" placeholder="0" required style="width: 100%; background: #1A1A1A; border: 1px solid #333; color: white; padding: 15px; border-radius: 15px; margin-top: 5px;">
            </div>
            <div style="margin-bottom: 25px;">
                <label style="font-size: 10px; color: #555; font-weight: 800;">TENGGAT WAKTU (DEADLINE)</label>
                <input type="date" name="deadline" required style="width: 100%; background: #1A1A1A; border: 1px solid #333; color: white; padding: 15px; border-radius: 15px; margin-top: 5px;">
            </div>
            <button type="submit" class="btn-submit">SIMPAN TARGET</button>
            <button type="button" onclick="closeGoalModal()" style="width: 100%; background: none; border: none; color: #444; margin-top: 15px; font-weight: bold; cursor: pointer; font-size: 0.8rem;">BATAL</button>
        </form>
    </div>
</div>
...
@section('scripts')
<script>
    // Fungsi Buka Modal Target
function openGoalModal() {
    document.getElementById('modalGoal').classList.add('active');
}

// Fungsi Tutup Modal Target
function closeGoalModal() {
    document.getElementById('modalGoal').classList.remove('active');
}

// Tambahkan juga agar klik di luar modal bisa menutup modalnya
window.addEventListener('click', function(event) {
    ...
    @foreach($goals as $goal)
        fetch(`/savings-advice/{{ $goal->id }}`)
            .then(res => res.json())
            .then(data => {
                document.getElementById('advice-{{ $goal->id }}').innerText = data.advice;
            });
    @endforeach
});
</script>
